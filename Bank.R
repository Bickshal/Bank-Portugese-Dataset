...
p_svm <- predict(s_fit,Test_Set,type = 'class')
confusionMatrix(Test_Set$y,p_svm)
plot(roc(Test_Set$y,as.numeric(p_svm)),print.auc = TRUE)
...
s1 <- svm.fs(y~.,data = Training_Set)
  
svm_fit <- svm(y~.,data = Training_Set)
summary(svm_fit)
predict_svm <- predict(svm_fit,Test_Set,type= 'response')
confusionMatrix(Test_Set$y,predict_svm)
plot(roc(Test_Set$y,as.numeric(predict_svm)),print.auc = TRUE)


svm_fit_oversample <- svm(y~.,data=Training_Set_Oversample)
summary(svm_fit_oversample)
predict_svm_os <- predict(svm_fit_oversample,Test_Set_Oversample,type = 'response')
confusionMatrix(Test_Set_Oversample$y,predict_svm_os)
plot(roc(Test_Set_Oversample$y,as.numeric(predict_svm_os)),print.auc=TRUE)

svm_fit_undersample <- svm(y~.,data=Training_Set_Undersample)
summary(svm_fit_undersample)
predict_svm_us <- predict(svm_fit_undersample,Test_Set_Undersample,type = 'response')
confusionMatrix(Test_Set_Undersample$y,predict_svm_us)
plot(roc(Test_Set_Undersample$y,as.numeric(predict_svm_us)),print.auc=TRUE)
...
#default chnaged from categorical to numeric
...
bank_knn$default <- as.integer(lapply(bank$default,def))

#education chnaged from categorical to numeric
...
bank_knn$education <- as.integer(lapply(bank$education,edx))


#marital chnaged from categorical to numeric
...
bank_knn$marital <- as.integer(lapply(bank$marital,marriage))

#job chnaged from categorical to numeric
unique(bank$job)
...
bank_knn$job <- as.integer(lapply(bank$job,job))

#housing chnaged from categorical to numeric
unique(bank_knn$housing)
...
bank_knn$housing <- as.integer(lapply(bank_knn$housing,housing))

#loan chnaged from categorical to numeric
unique(bank_knn$loan)
...
bank_knn$loan <- as.integer(lapply(bank_knn$loan,loan))

#contact chnaged from categorical to numeric
unique(bank_knn$contact)
...
bank_knn$contact <- as.integer(lapply(bank_knn$contact,contact))

#month chnaged from categorical to numeric
unique(bank_knn$month)
...
bank_knn$month <- as.integer(lapply(bank_knn$month,month))
#poutcome chnaged from categorical to numeric

unique(bank_knn$poutcome)
...
bank_knn$poutcome <- as.integer(lapply(bank_knn$poutcome,poutcome))
...
bank_knn$y <- as.integer(lapply(bank_knn$y,y))
...
bank_norm <- as.data.frame(lapply(bank_knn[,-17],data_norm))

summary(bank_norm[,1:4])     

n <- as.integer(0.75 * nrow(bank_norm))
n

training_set <- bank_norm[1:n,]
test_set <- bank_norm[(n+1):45211,]

library(class)

bank_pred <- knn(Training_Set,Test_set,bank[1:n,17] , k = 300)

table(bank_pred,bank[(n+1):45211,17])

confusionMatrix(bank[(n+1):45211,17],bank_pred)

plot(roc(bank[(n+1):45211,17],as.numeric(bank_pred)),print.auc=TRUE)
